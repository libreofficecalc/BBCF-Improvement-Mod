...
void ImGui::ColorTooltipIndexed(const char* text, int idx, const float* col, ImGuiColorEditFlags flags)
{
    ...
    ImGui::ColorButton("##preview", ImVec4(col[0], col[1], col[2], col[3]), (flags & (ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoTooltip, sz);
    ImGui::SameLine();
...
//	{
//		float mid_x = (float)(int)((bb_inner.Min.x + bb_inner.Max.x) * 0.5f + 0.5f);
//		RenderColorRectWithAlphaCheckerboard(ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col), grid_step, ImVec2(-grid_step + off, off), rounding, ImDrawFlags_RoundCornersTopRight | ImDrawFlags_RoundCornersBottomRight);
//		window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_without_alpha), rounding, ImDrawFlags_RoundCornersTopLeft | ImDrawFlags_RoundCornersBottomLeft);
//	}
//	else
//	{
//		// Because GetColorU32() multiplies by the global style Alpha and we don't want to display a checkerboard if the source code had no alpha
...
//			RenderColorRectWithAlphaCheckerboard(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), rounding);
//		else
//			//window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), rounding, ImDrawFlags ImDrawCornerFlags_All);
//			window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), rounding, ImDrawFlags_RoundCornersAll);
//	}
//	if (g.Style.FrameBorderSize > 0.0f)
//		RenderFrameBorder(bb.Min, bb.Max, rounding);
//	else
...
//		flags = (flags & (~ImGuiColorEditFlags_InputMask_)) | ImGuiColorEditFlags_InputRGB | ImGuiColorEditFlags_NoOptions;
//
//
...
//	flags |= (g.ColorEditOptions & ~(ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_DataTypeMask_ | ImGuiColorEditFlags_PickerMask_));
//
...
//
//	if ((flags & (ImGuiColorEditFlags_InputRGB | ImGuiColorEditFlags_InputHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
//	{
...
//		};
//		const int fmt_idx = hide_prefix ? 0 : (flags & ImGuiColorEditFlags_InputHSV) ? 2 : 1;
//
