...
PlayerFrameState::PlayerFrameState(scrState* state, unsigned int frame, CharData* player) {
  ...
  size_t len = state->active_ranges.size();
  ...
  if (player->blockstun) {
    kind |= BLOCKSTUN;
      ...
      kind |= HITSTUN;
  } else if (isDoingActionInList(state->name.c_str(), idleWords)) {
      kind &= IDLE;
  } else if (len != 0) {
    // HACK: kind of hacky :/, might change later
    for (size_t i = 0; i < len; i += 2) {
        if (state->active_ranges[i] <= frame && state->active_ranges[i + 1] >= frame) {
            kind |= ACTIVE;
            break;
        }
    }
    ...
    if ((kind & ACTIVE) == 0) {
      if (frame > state->active_ranges[len - 1]) {
        kind |= RECOVERY;
      } else {
        kind |= STARTUP;
      }
...
StatePair FrameHistory::getPlayerFrameStates(CharData *player1, CharData *player2) {
  // get the state
  // we need the amount of frames spent on the current state
  // Suppose we know the frame count for state n, if we changed states, start another count
  // otherwise, add the difference between our framecount, and the global frame count.
  // I could use the global frame count for this, or the global players, I am just going to write something.
  ...
  return {PlayerFrameState(p1_State, p1_frames, player1), PlayerFrameState(p2_State, p2_frames, player2)};
}

/// update the history queue with the new player states. Only call after the game time has moved
/// and by NO MORE than 1 frame
void FrameHistory::updateHistory() {
  ...
  if (states[0].kind == IDLE && states[1].kind == IDLE) {
